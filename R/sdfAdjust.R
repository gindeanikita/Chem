sdfAdjunst <- function(sdfFile, directory, saveNewFile, newFileName) {
  ...
  if (directory == NULL) { directory = getwd()}
  if (saveNewFile == NULL) { saveNewFile <- FALSE}
  if (newFileName == NULL) {newFileName <- paste(sdfFile, "_Treated.sdf", 
                                                 sep = "")}
